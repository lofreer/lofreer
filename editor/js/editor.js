(function (factory) {
	...
	UI.Dialog.prototype = {
		getHtmlTpl: function() {
			return UI.createEl('div', {id:'editor-fixedlayer',class:'editor-fixedlayer'}, [this.content], this.handles);
		},
		show: function(fn) {
			var dom;
	...
	Editor.prototype = {
		...
		setup: function(doc) {
			...
			self.body = doc.body;	
			self.options.toolbarWrap && (self.options.toolbarWrap = document.querySelector(self.options.toolbarWrap));
			self.initToolbar(self.options.toolbarWrap || self.container);			
			self.initEvents();					
			self.isReady = 1;
			self.emit('ready');
    				...
    				click: function() {
		    			...
		    			var menu = new Edit.ui.Dialog({
				    		content: Edit.ui.createEl('ul',{class:'menu-list menu-colors',style:Edit.ui.getPanelOffset(self.id)},Edit.utils.map(editor.options.colors,function(val,key){
				    			return Edit.ui.createEl('li',{class:'menu-item',title:key},[Edit.ui.createEl('span',{class:'eicon eicon-'+cmd,style:'color:'+val})],{click:function(ev){editor.execCommand(cmd,val);Edit.ui.closePopup();}})
				    		}),{
    				...
    				click: function() {
		    			...
		    			var menu = new Edit.ui.Dialog({
				    		content: Edit.ui.createEl('ul',{class:'menu-list',style:Edit.ui.getPanelOffset(self.id)},Edit.utils.map(editor.options[cmd+'s'],function(val,key){
				    			return Edit.ui.createEl('li',{class:'menu-item',title:key},[Edit.ui.createEl(val,[key])],{click:function(ev){editor.execCommand(cmd,val);Edit.ui.closePopup();}})
				    		}), {
								mousewheel: Edit.ui.bodyNotScroll
							})
				    	});
	...
	['h1', 'h2', 'h3', 'h4'].forEach(function(cmd){
		Edit.commands[cmd] = {
			execCommand: function(c,v) {
				this.document.execCommand('formatblock',false,v);
			},
    		...
    		var ui = new Edit.ui.Button({
    			...
    			handles: {
    				click: function() {
		    			...
		    			var menu = new Edit.ui.Dialog({
				    		content: Edit.ui.createEl('ul',{class:'menu-list',style:Edit.ui.getPanelOffset(self.id)},Edit.utils.map(editor.options[cmd+'s'],function(val,key){
				    			return Edit.ui.createEl('li',{class:'menu-item',title:val},[Edit.ui.createEl('span',{style:'font-family:'+val+''},[val])],{click:function(ev){editor.execCommand(cmd,val);Edit.ui.closePopup();}})
				    		}),{
								mousewheel: Edit.ui.bodyNotScroll
							})
				    	});
    		...
    		var ui = new Edit.ui.Button({
    			...
    			handles: {
    				click: function() {
		    			...
		    			var menu = new Edit.ui.Dialog({
				    		content: Edit.ui.createEl('ul',{class:'menu-list',style:Edit.ui.getPanelOffset(self.id)},Edit.utils.map(editor.options[cmd+'s'],function(val,key){
				    			return Edit.ui.createEl('li',{class:'menu-item',title:key,style:'font-size:'+key},[key],{click:function(ev){editor.execCommand(cmd,val);Edit.ui.closePopup();}})
				    		}), {
								mousewheel: Edit.ui.bodyNotScroll
							})
				    	});
				    		...
				    		content: ce('div',{class:'eui-dialog',style:Edit.ui.getPanelOffset(self.id)},[
				    			ce('div',{class:'panel-box'},[					    			
					    			ce('div',{class:'panel-content'},[	
				    					...
				    					ce('div',{class:'panel-handles'},[
				    						ce('span',{class:'panel-cancel'},['取消'],{click:function(){Edit.ui.closePopup()}}),
				    						ce('span',{class:'panel-confirm'},['确定'],{click:insertVideo})					    					
					    				])
					    			])
					    		])
				    		], {
